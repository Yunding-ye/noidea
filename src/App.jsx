...
export default function App() {
  ...
  const normalized = useMemo(() => {
    ...
    const manipulationTypes = Array.isArray(result.manipulationTypes) ? result.manipulationTypes : [];
    const specificEvidence = Array.isArray(result.specificEvidence) ? result.specificEvidence : [];
            ...
            <article className="card">
              ...
              <div className="chip-row">
                {normalized.manipulationTypes.length
                  ? normalized.manipulationTypes.map((type) => <span className="chip" key={type}>{type}</span>)
            ...
            <section className="full">
              ...
              <div className="evidence-grid">
                {normalized.specificEvidence.length ? normalized.specificEvidence.map((item, index) => (
                  <article className="card evidence-card" key={`${item.quote || 'evidence'}-${index}`}>
                    <p className="evidence-quote">“{item.quote || 'No quote provided.'}”</p>
                    <p><strong>Pattern:</strong> {item.pattern || 'Not specified'}</p>
                    <p><strong>Explanation:</strong> {item.explanation || 'Not specified'}</p>
                  </article>
